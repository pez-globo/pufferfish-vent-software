...
SCENARIO("Validate the pressure ", "[BMP388]") {
  GIVEN("Pressure") {
    ...
    PF::HAL::MockSPIDevice mockSPiDevice;
    PF::Driver::SPI::BMP388 test_bmp388(mockSPiDevice);
    PF::SPIDeviceStatus Spidevicestatus;

    /* Set pressure */
    mockSPiDevice.set_write_read(tx_buf, 5);
    WHEN("Invoke raw_pressure to read the pressure") {
      Spidevicestatus = test_bmp388.raw_pressure(pressure);
      THEN("SPI device shall return ok"){
        REQUIRE(Spidevicestatus == PF::SPIDeviceStatus::ok);
  ...
  GIVEN("Pressure  is not able to read from sensor - read_error") {
    ...
    WHEN("Invoke raw_pressure to read the pressure") {
      Spidevicestatus = test_bmp388.raw_pressure(pressure);
      THEN("SPI device shall return read error"){
        REQUIRE(Spidevicestatus == PF::SPIDeviceStatus::read_error);
      }
    }
  }
}
...
SCENARIO("Validate the temperature ", "[BMP388]") {
  GIVEN("Temperature") {
    ...
    WHEN("Invoke raw_temperature to read the temperature") {
      Spidevicestatus = test_bmp388.raw_temperature(temperature);
      THEN("SPI device shall return ok"){
        REQUIRE(Spidevicestatus == PF::SPIDeviceStatus::ok);
...
SCENARIO("Read calibration data ", "[BMP388]") {
  GIVEN("Calibration data") {
    const uint8_t tx_buf[23] = {0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
        0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
        0x01, 0x01, 0x01};
    PF::Driver::SPI::TrimValues data;

    uint16_t temperature1 = (static_cast<uint16_t>(tx_buf[3] << 8) | static_cast<uint16_t>(tx_buf[2]));
    uint16_t temperature2 = (static_cast<uint16_t>(tx_buf[5] << 8) | static_cast<uint16_t>(tx_buf[4]));
    int8_t temperature3 = (static_cast<int8_t>(tx_buf[6]));

    int16_t pressure1 = static_cast<int16_t>(static_cast<uint16_t>(tx_buf[8] << 8) | static_cast<uint16_t>(tx_buf[7]));
    int16_t pressure2 = static_cast<int16_t>(static_cast<uint16_t>(tx_buf[10] << 8) | static_cast<uint16_t>(tx_buf[9]));
    ...
    int16_t pressure9 = (static_cast<int16_t>(static_cast<uint16_t>(tx_buf[20] << 8) | static_cast<uint16_t>(tx_buf[19])));
    ...
    WHEN("Invoke read_calibration_data to read the calibration data") {
      Spidevicestatus = test_bmp388.read_calibration_data(data);
        THEN("SPI device shall return ok"){
          REQUIRE(Spidevicestatus == PF::SPIDeviceStatus::ok);
...
SCENARIO("Read calculated calibration coefficients ", "[BMP388]") {
  GIVEN("Calibration coefficients") {
    const uint8_t tx_buf[23] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A,
                                0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14,
                                0x15, 0x16, 0x17};
    PF::Driver::SPI::CalibrationData data;

    double temperature1 = static_cast<double>(static_cast<uint16_t>(tx_buf[3] << 8) | static_cast<uint16_t>(tx_buf[2])) / pow(2, -8);
    double temperature2 = static_cast<double>(static_cast<uint16_t>(tx_buf[5] << 8) | static_cast<uint16_t>(tx_buf[4])) / pow(2, 30);
    double temperature3 = static_cast<double>(static_cast<int8_t>(tx_buf[6])) / pow(2, 48);

    double pressure1 = (static_cast<double>(static_cast<int16_t>(static_cast<uint16_t>(tx_buf[8] << 8) | static_cast<uint16_t>(tx_buf[7])) - pow(2, 14)) / powf(2, 20));
    double pressure2 = (static_cast<double>(static_cast<int16_t>(static_cast<uint16_t>(tx_buf[10] << 8) | static_cast<uint16_t>(tx_buf[9])) - pow(2, 14)) / powf(2, 29));
    ...
    double pressure9 = (static_cast<double>(static_cast<int16_t>(static_cast<uint16_t>(tx_buf[20] << 8) | static_cast<uint16_t>(tx_buf[19]))) / pow(2, 48));
    ...
    WHEN("Invoke read_compensate_temperature to read the compensated temperature") {
      Spidevicestatus = test_bmp388.calc_calibration_coefficient(data);
        THEN("SPI device shall return ok"){
          REQUIRE(Spidevicestatus == PF::SPIDeviceStatus::ok);
          REQUIRE(data.par_t1 == temperature1);
          REQUIRE(data.par_t2 == temperature2);
...
SCENARIO("Read compensate temperature ", "[BMP388]") {
  GIVEN("Compensated temperature data") {
    const uint8_t tx_buf[23] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
    ...
    double part1 = static_cast<double>(static_cast<uint16_t>(tx_buf[3] << 8) | static_cast<uint16_t>(tx_buf[2])) / pow(2, -8);
    double part2 = static_cast<double>(static_cast<uint16_t>(tx_buf[5] << 8) | static_cast<uint16_t>(tx_buf[4])) / pow(2, 30);
    ...
    mockSPiDevice.set_write_read(tx_buf, 23);
    WHEN("Invoke read_compensate_temperature to read the compensated temperature") {
      Spidevicestatus = test_bmp388.read_compensate_temperature(compensated_temperature);
      THEN("SPI device shall return ok"){
        REQUIRE(Spidevicestatus == PF::SPIDeviceStatus::ok);
...
SCENARIO("Read compensate pressure ", "[BMP388]") {
  GIVEN("Compensated pressure data") {
    ...
    part1 = (static_cast<double>(static_cast<int16_t>(static_cast<uint16_t>(tx_buf[8] << 8) | static_cast<uint16_t>(tx_buf[7])) - pow(2, 14)) / powf(2, 20));
    part2 = (static_cast<double>(static_cast<int16_t>(static_cast<uint16_t>(tx_buf[10] << 8) | static_cast<uint16_t>(tx_buf[9])) - pow(2, 14)) / powf(2, 29));
    ...
    double part9 = (static_cast<double>(static_cast<int16_t>(static_cast<uint16_t>(tx_buf[20] << 8) | static_cast<uint16_t>(tx_buf[19]))) / pow(2, 48));
    ...
    WHEN("Invoke read_compensate_pressure to read the compensated pressure") {
      Spidevicestatus = test_bmp388.read_compensate_pressure(compensated_pressure);
      THEN("SPI device shall return ok"){
        REQUIRE(Spidevicestatus == PF::SPIDeviceStatus::ok);
